...
export async function setAddress(
  ...
  try {
    ...
    const addressTrigger = p.locator(
      '[data-testid="delivery-address-input"], [class*="deliveryAddress"], input[placeholder*="address" i], input[placeholder*="Enter" i]'
    ).first();
    ...
    const addressInput = p.locator(
      'input[placeholder*="address" i], input[placeholder*="Enter" i], [data-testid="delivery-address-input"]'
    ).first();
    ...
    const suggestion = p.locator(
      '[class*="autocomplete"] li, [class*="suggestion"], [role="option"], [class*="AddressSuggestion"]'
    ).first();
    ...
    const confirmBtn = p.locator(
      'button:has-text("Search"), button:has-text("Done"), button:has-text("Confirm"), button:has-text("Find Food")'
    ).first();
...
export async function searchRestaurants(
  ...
  try {
    ...
    const cards = p.locator(
      '[class*="restaurantCard"], [class*="restaurant-card"], [data-testid*="restaurant"], li[class*="restaurant"]'
    );
...
export async function checkout(
  ...
  success: boolean;
  orderId?: string;
  summary?: { items: CartItem[]; total: number; deliveryAddress?: string; estimatedDelivery?: string };
