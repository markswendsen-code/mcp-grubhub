...
import { checkAuth, login, setAddress, searchRestaurants, getRestaurant, addToCart, viewCart, clearCart, checkout, trackOrder, getOrders, cleanup, } from "./browser.js";
import { hasStoredCookies, clearCookies, getCookiesPath } from "./auth.js";
const server = new Server({ name: "strider-grubhub", version: "0.1.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: "grubhub_status",
                description: "Check if the user is logged in to Grubhub. Returns auth status and instructions if not authenticated. Call this before any other Grubhub operations.",
                inputSchema: { type: "object", properties: {} },
            },
            {
                name: "grubhub_login",
                description: "Log in to Grubhub with email and password via browser automation. Session is saved for future use.",
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            },
            {
                ...
                description: "Set the delivery address for Grubhub orders. Must be set before searching for restaurants.",
                inputSchema: {
                    type: "object",
                    properties: {
                        address: {
            ...
            },
            {
                ...
                description: "Search for restaurants on Grubhub by name, food type, or cuisine. Returns a list of restaurants with ratings, delivery times, and fees.",
                inputSchema: {
                    type: "object",
                    properties: {
                        query: {
            ...
            },
            {
                ...
                description: "Get full details for a restaurant: menu categories, items with prices, hours, and address.",
                inputSchema: {
                    type: "object",
                    properties: {
                        restaurantId: {
            ...
            },
            {
                ...
                description: "Proceed to checkout. Set confirm=false to preview order details before placing; set confirm=true to actually place the order.",
                inputSchema: {
                    type: "object",
                    properties: {
                        confirm: {
            ...
            },
            {
                ...
                description: "Track the status of an active Grubhub order — delivery progress, estimated time, and driver info.",
                inputSchema: {
                    type: "object",
                    properties: {
                        orderId: {
